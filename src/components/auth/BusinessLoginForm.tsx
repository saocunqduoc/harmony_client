...
const BusinessLoginForm = () => {
  ...
  const onSubmit = async (values: LoginFormValues) => {
    setError(null);
    // Truyền vai trò 'business' để chỉ ra đây là đăng nhập doanh nghiệp
    const success = await login(values.email, values.password);
    
    if (success) {
      // Sau khi đăng nhập thành công, kiểm tra xem người dùng có phải là doanh nghiệp
      // và chuyển hướng đến trang quản trị doanh nghiệp
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              <FormItem>
                ...
                <FormControl>
                  <div className="relative">
                    ...
                    <Input
                      placeholder="doanhnghiep@example.com"
                      className="pl-10"
                      {...field}
                    />
                  </div>
          ...
          <Button type="submit" className="w-full" disabled={isLoading}>
            {isLoading ? 'Đang đăng nhập...' : 'Đăng nhập với tư cách doanh nghiệp'}
          </Button>
        </form>
      </Form>
    </div>
